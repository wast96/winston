...
Status: Step 0 done and approved. Batches 1-12 (ch01-ch12) COMPLETE: translated,
checked, footnoted, built, QA green, committed. Next is Batch 13 (ch13). 12
...
was `Chang'an B12`; the next is `Chang'an B13`). CLAUDE.md records the rule; the
...
out/The Longest Day in Chang'an.epub. Step 0 and Batches 1-12 (ch01-ch12) are
done; the 25-batch plan (one chapter per batch) is approved.

Do Batch 13 = ch13 (第十三章 亥正 / "Chapter Thirteen. The Hour of the Pig, Second
Half (10 p.m.)") end to end. It is ~16,497 source chars. NOTE: data/src/ and
...
batch's source from its text_file in book.json (data/src/28_text00027.txt); the
source is authoritative, quote it verbatim in the bilingual QC file and render it
faithfully and in full. Author one aligned bilingual QC file out/ch13_bilingual.md
...
as in ch01-ch12; render the source's per-chapter time-gloss final line as the
...
into one bilingual pair (ch07-ch12 each merged the opening vignette's and the
dateline's split halves; a quick scan: flag any source line whose last char is not
in 。！？"）…— nor a colon). Then generate out/ch13_reading.md and the parity source
with `scripts/split_bilingual.py out/ch13_bilingual.md ch13 "第十三章　亥正"` (use the
exact full-width-space zh title from book.json). Run
`scripts/check_numbers.py out/ch13_bilingual.md --noise noise.txt` (extend
...
`scripts/check_structure.py --pairs data/zh/ch13.txt out/ch13_reading.md` (parity
...
under key "ch13" (verbatim English anchors; XHTML bodies with numeric character
references for punctuation/accents, literal CJK for Chinese terms is fine and
builds — ch01/ch09/ch10/ch11/ch12 do it — never HTML named entities; ~3 per
chapter, recurring subjects get their note at first appearance across the whole
book, so skip anything already noted in ch01-ch12). Add any figure specs to
...
pending-aware TOC links ch01-ch13 content and every other chapter's skeleton, then
...
error rate in PROGRESS.md. Rewrite HANDOFF.md with the Batch 14 (= ch14) kickoff
message (its fenced block opening with the line `Chang'an B14`), commit, and push
...
- Batch 11 = ch11, complete: 3 notes (43 total), glossary grown by 20 rows,
  noise.txt extended, qa PASS.
- Batch 12 = ch12, complete and committed: out/ch12_reading.md, data/zh/ch12.txt,
  3 notes (46 total: 管仲祠/the Guan Zhong shrine; 鱼肠/Yuchang the dagger-name;
  卑路斯/Peroz the last Sasanian heir). glossary.json grown by ~15 rows: people
  鱼肠/Yuchang, 卑路斯/Peroz, 太宗/Taizong, 高宗/Gaozong, 管仲/Guan Zhong, 老聃/Lao
  Dan, 燕子李/Li the Swallow, 老赵/Old Zhao; org 右威卫/the Right Awesome Guard;
  places 波斯/Persia, 管仲祠/the Guan Zhong shrine; terms 四望车/four-windowed
  carriage, 甩霞舞/the Rosy-Cloud Fling, 绿矾油/green-vitriol oil, 跑窟/cave-running.
  noise.txt extended (六个字, 说三道四, 三步并两步; the one real count 两边必须选一边
  carried in the ENGLISH as "of the two he had to choose one"). qa PASS (46 notes).
  Verbatim-quote check 375/375 clean; blind double-translation (Zhang's despair)
  and back-translation (the flash-forward vignette) both clean, 0 errors.

## What is NEXT

- Batch 13 = ch13 (第十三章 亥正, ~16,497 source chars, data/src/28_text00027.txt).
  Then B14=ch14 ... B24=ch24, B25=ch25+ch26. See book.json's structure/batches.

## House style set by Batches 1-12 (follow it)
...
- Openings: NOT every chapter has an epigraph. Each chapter's opening differs
  ...
  carriage vignette each recur later and were translated identically). The
  content-file time-marker heading line (亥初 etc.) is absorbed into the H2
  chapter title, not made a paragraph. The source's per-chapter time-gloss (its
...
- Scene breaks: the source divides multi-scene chapters with a decorative rule
  ...
  matching ch01-ch12. If visible scene breaks are ever wanted, that is a global
...
- Names: pinyin, one decided rendering per referent, all in glossary.json. Grep
  ...
  ch01-ch12 that MUST be reused verbatim include: Zhang Xiaojing, Li Bi
  ...
  Xiangxian), Liu Shiqi (刘十七), Mojialuo (摩伽罗), and the Batch-12 cast: Yuchang
  (鱼肠, the assassin), Peroz (卑路斯), Old Zhao (老赵, the post-soldier), Guan Zhong
  (管仲), Lao Dan (老聃), Li the Swallow (燕子李). Orgs: the Jing'an Bureau, the
  Lüben Guards, the Jinwu Guard, the Right Xiao Guard / Leopard Cavalry / Sixteen
  Guards of the Southern Command, the Right Awesome Guard (右威卫), the Court of
  ...
  Pingkang Quarter 平康里, Guangde, Changming, Xuanyang, Yongle, Changxing, Anren),
  the Xingqing Palace + the Qinzheng Wuben Tower, the Cibei/Daqin/Persian Temples
  + the Guan Zhong shrine, the ten frontier commands, Balhuan, Yanzhou, Tianzhu
  ...
  Sogdian Whirl, ...), the ch10 cluster, the ch11 cluster (firepoint 火点,
  firemaster 火师, the Self-Raining Pavilion, storax, ...), and the ch12 cluster
  (green-vitriol oil 绿矾油, cave-running 跑窟, the four-windowed carriage 四望车,
  the Rosy-Cloud Fling 甩霞舞).
- Titles/address: 太子/东宫 = "heir apparent" / "the Eastern Palace"; 司丞 (Li Bi,
  ...
  Patrol", 端公 = "Duangong", 副端 = "Vice-Duan"; 京兆尹 = "the Prefect of
  Jingzhao", 中书令 = "the Secretariat Director". 工部/虞部/大理寺/御史台/刑部/内侍省/
  仓曹/中书省/祠部/右威卫 per Hucker (see glossary).
- Numbers: run check_numbers with --noise noise.txt every batch. When it flags a
  ...
  list enumerator, a book/office/game-name numeral, a character-COUNT like
  四个字/六个字), extend noise.txt (own-line comments) or WORD_NUM, and say so in
  ...
  had to choose one"). A genuinely dropped number must still fail. When a name's
...
- Working branch is claude/the-longest-day-in-changan; push only there. Do not
  ...
  CLAUDE.md rule 2 and the commissioner override it. B06-B12 were each started on
  a stray per-batch branch and all work was consolidated onto
  claude/the-longest-day-in-changan, the remote's canonical branch. B12
  specifically: the session opened on claude/the-longest-day-in-changan-ytt638;
  its HEAD equaled origin canonical (Batch 11), so the canonical branch was
  checked out and reset to origin, the work was done there, and the stray local
  branch was deleted; the stray remote ytt638 was deleted too.)
...
- The bilingual QC file never ships (and is not committed). Note anchors must be
  ...
  is fine (ch01/ch09/ch10/ch11/ch12), numeric character references for typographic
...
- Extractor artifacts: a logical paragraph is sometimes split across two lines in
  ...
  one bilingual pair (ch07-ch12 merged the opening vignette's two lines and the
  dateline's two lines; ch12 also merged Zhang's split speech, source lines
  347-348, whose first half ended on an em-dash). A quick way to find them: scan
  for source lines whose last char is not in 。！？"）…— nor a colon. (Multi-paragraph
...
- Watch for authorial slips (ch03 祆正-for-Sabao and a tutor date; ch04 麻格心 for
  ...
  with pre-dawn 卯 content). Render the intent for a mis-named established
  referent; render genuine source errors faithfully and visibly (rule 4) and flag
  them in PROGRESS. NOTE: ch12's time-gloss is CORRECT (亥 glossed with "晚上9点 …
  21时至23时"); do not assume every hour-note is broken.
- A footnote's subject gets its note at its FIRST appearance in the whole book.
  ...
  Peroz, and everything in notes.json ch01-ch12.

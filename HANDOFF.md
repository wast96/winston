...
Read CLAUDE.md in full (the working rules at the top are non-negotiable), then HANDOFF.md, then book.json. We are translating 英雄无名 (Nameless Heroes) by Chen Gongshu, a Nationalist/Juntong secret-service memoir, from a digital EPUB (source.epub) into an annotated English EPUB, following CLAUDE.md exactly. Work ONLY on branch claude/nameless-heroes; expect the harness to start you on a stray per-task branch and consolidate per CLAUDE.md rule 2 (check out claude/nameless-heroes, reset to origin, carry over any stray commits, delete the stray local and remote). Deliverable: out/nameless-heroes.epub. Run ./setup.sh once (its ONE failing regression test, "hook stands down on template stub", is a KNOWN false alarm; all others pass), then re-ingest with scripts/ingest_epub.py source.epub (data/src is gitignored/regenerable). B01 (front matter, ch01-ch05), B02 (ch06), B03 (ch07), B04 (ch08), B05 (ch09), B06 (ch10 preface + ch11), B07 (ch12), B08 (ch13), B09 (ch14), B10 (ch15), B11 (ch16), B12 (ch17), B13 (ch18 + ch19), B14 (ch20), B15 (ch21), B16 (ch22), B17 (ch23), B18 (ch24), B19 (ch25), B20 (ch26) and B21 (ch27) are DONE; the voice gate is PASSED and the FROZEN register reference is reference/B01_frozen.md. Do NOT re-do them. PART TWO ("Disgrace at Hanoi") is COMPLETE; PART THREE ("Renown Won in a Hundred Battles" / 百战声威) is under way (ch20 self-preface + ch21/ch22/ch23/ch24/ch25/ch26/ch27). The EPUB now holds 27/43 chapters, 259 notes. NOTE on batch numbering: book.json's batches array lumps ch23+ch24 as "B17", so the working batch labels run ONE AHEAD of the book.json array from ch24 on (ch24 = B18, ch25 = B19, ch26 = B20, ch27 = B21, ch28 = B22).

Do Batch B22 = ch28 (ONE unit, a FULL chapter): ch28 = 第九章 声威大震血浪腥风 "Chapter 9. Fearsome Renown, Waves of Blood" (confirm title_en in book.json). ch28 continues DIRECTLY from ch27's tail: ch27 ("The Death of a Tycoon") closed on the puppet regime's late-1940/early-1941 institutional offensive - 周佛海 (Zhou Fohai) masterminding the seizure of the concessions' 特区法院 "Special District Court" and the setting-up of the puppet 中央储备银行 "Central Reserve Bank" - and the 汪伪 bloody reprisal that harmed scores of innocents. ch28 (声威大震 血浪腥风 "Fearsome Renown, Waves of Blood") is the treatment of that height-of-renown-and-blood period, and it REPRODUCES the two court-retrocession/會審公廨 agreements (公共租界 + 法租界) whose 特区法院 topic ch27's tail raised. Read the tail of ch27 English (out/ch27_reading.md, section 3's close on 特区法院/中央储备银行/血腥报复) and ch27 for register + story continuity. Run it end to end per the CLAUDE.md pipeline, to completion (no approval gate):
1. Read ch28 from data/src (29_index-split-000-0027.txt). CONFIRM structure p-by-p against data/src_epub/OEBPS/Text/index_split_000_0027.xhtml [parses to 1 <h2> + 224 <p>, NO <h1>, NO <br/>, NO <img>, NO [\d+] note markers]. drop=2 (running header 英雄无名-陈恭澍 + <h2> chapter title). The txt is ~225 wc-l lines; after drop=2 the 224 body lines map 1:1 to the 224 <p> (NO <br/>, so NO intra-<p> line breaks - like ch27). Do the byte-exact p-by-p diff FIRST (the B19/B20/B21 method: extract <p> inner text, walk each <p> consuming 1 body line, assert every <p> matches its body line) to CONFIRM 224=224 and to LOCATE any SEVERED-<p> boundaries (a source <p> whose last char is non-terminal, continuing into the next <p> - ch25/ch26 had 7 each, ch27 had 2; per CLAUDE.md they MERGE, and parity is data/zh↔reading.md, NOT the raw <p> count). HEADS UP: ch28 uses the same ENUMERATED 一、二、三 SECTION HEADINGS as ch27 - THREE of them: p#0 一、一个特务工作者的心态与感受 (standalone), p#46 二、铲除巨奸寒敌胆树立声威, p#126 三、谁来清偿这笔寃孽债. Determine p-by-p whether p#46/p#126 are standalone or TAIL-GLUED (watch the "three-tell": standalone; tail-glued ending non-terminal OR in a full-width 」; head-glued). CRUCIAL: ch28 ALSO contains INNER enumerated 一、二、三 DOCUMENT-CLAUSE lists that are NOT section headings - the two reproduced court agreements (p#135-137 the 公共租界 courts agreement, p#141-143 the 法租界 会审公廨/地方法院 agreement) run 一、二、三 as numbered CLAUSES of a quoted document; render these as ORDINARY paragraphs per parity (like ch27's (1)-(19) rebuttal list), NOT as ### headings. Judge by function: a section heading introduces a section; a clause is one item of a quoted agreement under a lead-in.
2. Extend scripts/clean_batch.py with ch28's spec (drop=2; the confirmed severed-<p> merges; the 3 一、二、三 SECTION headings as standalone/tail-glued; the inner document-clause 一、二、三 lists stay as ordinary body lines, NOT in `standalone`). It supports merges (chains follow the merge_from map), standalone, tail-glued `glued`, and head-glued `glued_head`. Run it (source-conservation check must pass). Write out/ch28_reading.md (## from book.json title_en; one English paragraph per source body line; the 3 section headings as ### ; document-clause list items as ordinary paragraphs per parity). Then run scripts/batch_artifacts.py ch28, and ALWAYS finish with a NO-ARG run (the trap: an ID-run writes checks.json with ONLY that unit; the no-arg run restores all 28 units so check_structure/check_content see them).
3. Translate to the FROZEN register (Chen's voice sheet in HANDOFF; document-heavy chapters run higher - ch24 5.33, ch25 4.97, ch26 4.98, ch27 4.82 median; the narrating "shall" rises with quoted directives/documents - ch28 reproduces two formal court agreements, so expect it back; read the note, do NOT reset or de-formalize the deliberate "shall"). Consult glossary.json and authority.json BEFORE romanizing anything; REUSE the settled Part-Three renderings (see "Renderings settled" + the B15-B21 shelf sections of HANDOFF - all keyed with pinyin where keyed): 张啸林 Zhang Xiaolin (NOTED ch04), 林怀部 Lin Huaibu (keyed B20), 傅筱庵 Fu Xiao'an (keyed/NOTED ch04), 杜月笙 Du Yuesheng (NOTED ch17), 黄金荣 Huang Jinrong (keyed B21), 周佛海 Zhou Fohai (keyed; NOTED ch27), 李士群 Li Shiqun (ch04/ch17), 维新政府 the Reformed Government (NOTED ch27); the Shanghai District; the Juntong/the Juntong Bureau; 制裁 "sanction"; 敌伪 "the enemy and the puppets"; 特工总部/七十六号 "Special Operations Headquarters"/"No. 76"; 公共租界 "International Settlement"/法租界 "French Concession"; 工部局 "Municipal Council"/公董局 "French Municipal Council" (NOTED ch27); 特区法院 "Special District Court" (NOTED ch27); 中央储备银行 "Central Reserve Bank" (NOTED ch27); 会审公廨 "the Mixed Court" (the 特区法院's predecessor - the two agreements are about replacing it; render "the Mixed Court"); 第二行动大队 "Second Action Brigade". IMPORTANT place-name convention (the check_content/qc_entities gate): keyed CITY/PROVINCE names render in PINYIN per the glossary - 北平 Beiping, 天津 Tianjin, 汉口 Hankou, 四川 Sichuan, 虹口 Hongkou, 重庆 Chongqing (NOT postal forms; 重庆大公报 = "Chongqing Ta Kung Pao"); 愚园路 Yuyuan Road KEYED; non-keyed attested Shanghai ROADS keep their historical names (Route Wagner 华格臬路, Avenue Foch 福煦路, Yates Rd 同孚路, Avenue Joffre 霞飞路, Robison Road 劳勃生路, etc.; use pinyin only for the genuinely uncertain, e.g. 善钟路 Shanzhong Road). Render Republican years literally (二十九年 = "the twenty-ninth year"; the checker matches the source numeral or auto-escapes via +1911). WATCH ch28's digitization glitches (list in PROGRESS.md, render to plain sense, footnote only real reading uncertainty): same classes seen ch15-ch27 (single-char substitutions, dropped stops, dittography, mismatched guillemets, stray ？, u-for-「, and any ○/× redactions in room/lane/phone numbers - the numeric checker mis-reads ○; carry the real value in English and noise only the mis-read glyph-string; × redactions render as em-dash blanks). Dates/counts: carry real values as DIGITS/words; NOISE only idiom/approximate/name-numeral/elided/date-name forms (data/noise.txt already carries the B01-B21 rules incl. 八一三/八一四/十数/阿四; add ch28's). NOTE the chapter's own opening tallies (自序 figures: 一百余 traitors, 四十余 Japanese troops, 五十余 sabotage jobs, 两百余 own losses) - render "over a hundred / over forty / over fifty / over two hundred" and noise the approximate 余-forms as the checker flags them.
4. Checks: verify_unit.py ch28 (parity + numbers with noise auto-found + anchors); check_align.py ch28; regenerate checks.json with scripts/batch_artifacts.py (no args) and run check_structure.py --config checks.json + check_content.py --config checks.json (NOTE: check_content prints KNOWN PRE-EXISTING artifacts and exits nonzero because of them - ch08 Shunde ×3, ch13 ×9, ch09 "Jize County" ×1, and ch26's TWO documented keyed-substring FALSE POSITIVES 武汉卿/劳勃生路; CONFIRM ch28 itself shows "all in the paired paragraph" / 0 displaced, and align any keyed name/place to its glossary-decided rendering. A NEW unit's displacements are almost always a keyed name/place rendered a DIFFERENT way than the glossary - align the English to the keyed form; the exception is a keyed name that is a SUBSTRING of a larger different referent, a documented false positive, not a fix). Do NOT add COMMON-NOUN or book/periodical keys. qc_entities.py on a reconstructed bilingual (data/zh body lines + out/ch28_en.json, `> zh` / en pairs, strip the ### heading lines; every glossary row needs a pinyin field). Verify the TAIL against the source. check_register.py --ref reference/B01_frozen.md out/ch28_reading.md ("shall" is deliberate - read the note, do not de-formalize).
5. Footnotes per the reader model, first-appearance-disciplined with the greps and the NOT-re-noted ledger (full list in PROGRESS.md; the big already-covered furniture: No.76/特工总部/丁默邨/李士群 ch04/ch17, the concessions ch04, 制裁, the Green-Gang three tycoons + 张啸林 ch04, Du Yuesheng ch17, 忠义救国军 ch21, the 孤岛 Solitary Island ch26, the 八一三 Battle of Shanghai ch27, the two municipal councils ch27, the Reformed Government ch27, the Central Reserve Bank/Special District Court/Zhou Fohai ch27, fabi ch21, the Marco Polo Bridge ch13). ch28 (声威大震 血浪腥风) is the renown-and-bloody-reprisal chapter that reproduces the two court agreements - expect NEW furniture (会审公廨 the Mixed Court; the specific reprisal victims/incidents; any period figures/institutions) that earns notes; be generous but do NOT pad, do NOT re-note. Merge notes via apparatus_merge.py (numeric character references only in note bodies; anchors verbatim ASCII substrings of the reading.md body text, NO em dash and NO quote/apostrophe character - substring traps; multi-occurrence anchors attach at the first). Add glossary rows BY HAND into the sectioned glossary.json (idempotent + re-read-verified, every row with a pinyin field; apparatus_merge's glossary path assumes a FLAT map and would corrupt the sectioned file - use it ONLY for notes; scripts/add_ch27_glossary.py is the by-hand pattern, asserting each hanzi key against data/zh). Confirm ch28 carries no images (its XHTML has NO <img> - confirm). For any CJK in a note body use the make_ch27_apparatus.py pattern (author bodies with typed hanzi, ASSERT every non-ASCII glyph is present in data/zh/ch28.txt, then convert to NCRs) to defeat the CJK-mangling hazard - and remember a CORRECT glyph may be ABSENT if the source prints a glitch/variant form, so describe such terms with the source's own form + pinyin.
6. Rebuild the EPUB (scripts/build_reading_epub.py), qa_epub.py until green, epubcheck if available (jar at /tmp/epubcheck-5.1.0/epubcheck.jar; re-run setup.sh per session); record all check results in PROGRESS.md; update HANDOFF.md; commit and push to claude/nameless-heroes. (next is B23 = ch29; confirm its title_en in book.json - it is 第十章 (上), a two-part chapter split ch29/ch30.)

End with the TWO chat deliverables in the SAME final reply (CLAUDE.md banner): the rebuilt out/nameless-heroes.epub ATTACHED as a file, and the Batch B23 kickoff message pasted VERBATIM in a fenced code block. Cite chapters and sections, never pages. Do not pause for approval mid-batch.
```
...
- **Batch B20 (ch26), Part Three Chapter 6.** 第六章 泰山鸿毛 同此一掷 "Mount Tai or a Feather, All
  ...
  body paragraphs; 11 notes; 27 net new glossary rows (25 people + 2 orgs). Two documented
  keyed-substring FALSE POSITIVES in check_content/qc (武汉卿/劳勃生路). Heavily-corrupted source
  block (p#120-125) rendered to reconstructed sense + footnoted. Detail in PROGRESS.md ("Batch B20").
- **Batch B21 (ch27), Part Three Chapter 8.** 第八章 大亨之死 扑朔迷离 "The Death of a Tycoon,
  Shrouded in Mystery" - the 张啸林 (Zhang Xiaolin) tycoon-death case (killed 14 Aug 1940 by his
  bodyguard 林怀部 Lin Huaibu), continuing ch26's tail. Part Three SKIPS 第七章 (a FAITHFUL numbering
  gap: ch26 = 第六章, ch27 = 第八章). Chen's own doubts about whether the case was really the Juntong's
  doing; the two reproduced newspaper reports (新申报, 重庆大公报); the point-by-point demolition of a
  forged "confession letter" (林怀郭) published in a Taipei magazine. **drop=2; 1 <h2> + 136 <p>, NO
  <br/>/<img>/note-markers, byte-exact p-by-p; 2 severed-<p> merges (13/14, 41/42); 3 ENUMERATED
  一、二、三 section headings (1 standalone L3 + 2 tail-glued L64/L94, L94 ending in `」`).** 133 body
  paragraphs; 6 notes (259 cumulative); 2 net new glossary rows (赵圣 Zhao Sheng, 黄金荣 Huang
  Jinrong). check_content 0 displaced; qc 0 misses. All checks green; qa_epub PASS; epubcheck 0/0/0/0.
  **EPUB now 27/43 chapters.** Detail in PROGRESS.md ("Batch B21").
...
- **B21 shelf (ch27; reuse; keyed with pinyin):** 赵圣 Zhao Sheng (working name of the Second Action
  ...
  (third Green-Gang tycoon). REUSED keyed cast: 张啸林, 林怀部, 杜月笙, 傅筱庵, 潘子欣, 齐庆斌, 刘原深,
  郑修元, 万墨林, 梁鸿志, 陈群, 周佛海, 李士群, 何行健, 维新政府. NEW ch27 notes: the 八一三 August
  Thirteenth Incident (Battle of Shanghai, 1937); the two municipal councils (工部局 Shanghai
  Municipal Council / 公董局 French Municipal Council; the paper's 公部局 an error); the Reformed
  Government (维新政府, Liang Hongzhi/Chen Qun); the 18-vs-19 rebuttal-count slip; the 长奇/长崎-Nagasaki
  homophone; the Central Reserve Bank + Special District Court + Zhou Fohai. Rendered INLINE (not
  keyed): 东郭牙 Dongguo Ya (pen name), 裴可权 Pei Kequan, 马龙 Maron, 柳乃夫 Liu Naifu, 阿四 Ah Si, 朱升
  Zhu Sheng (the Fu Xiao'an cook-assassin, act NOTED ch04), 吴金桂/吴建臣/吴鸿 (the second victim named
  three ways). Attested roads inline: Route Wagner 华格臬路, Avenue Foch 福煦路, Yates Rd 同孚路,
  Avenue Joffre 霞飞路; 善钟路 rendered pinyin "Shanzhong Road" (Western name not confidently attested).
...
- REGISTER: educated, formal, essayistic first person; grave and a touch archaic but not stilted.
  ...
  B08 29%, B12 43%, B15 33%, B16 36%, B18 25%, B19 67%, B20 83%, B21 0% - elevated when the chapter
  reproduces many quoted directives/documents; ch26 carries three Dai Li telegrams + a formal
  memorial essay, whereas ch27 carries no directives at all - just Chen's argument, two news reports,
  and a forged letter - so the narrating "shall" simply does not arise there; NOT a de-formalization).
...
- Ratio ~4.55-4.78 en/han in narrative; prefaces denser (~5.2-5.3); document-heavy chapters run
  higher (ch21 4.89, ch22 4.70, ch24 5.33, ch25 4.97, ch26 4.98, ch27 4.82 median). Read the note, do not reset.
...
- **Part Three ("Renown Won in a Hundred Battles" / 百战声威) under way (B14-B21).** ch20 =
  ...
  living sacrifice, the enemy tally + Japanese gendarmerie record, the Kang Corps); ch27 = the
  张啸林 tycoon-death case (Chen's doubts, the two newspaper reports, the demolition of a forged
  confession-letter), closing on the puppet Central Reserve Bank / Special District Court offensive.
- **NEXT: B22 = ch28** - 第九章 声威大震血浪腥风 "Fearsome Renown, Waves of Blood". Continues ch27's
  tail (特区法院/中央储备银行/血腥报复); reproduces the two court-retrocession/會審公廨 agreements
  (公共租界 + 法租界). 1 <h2> + 224 <p>, NO <br/>, NO <img>, NO note markers. drop=2. THREE enumerated
  一、二、三 SECTION headings (p#0/p#46/p#126) PLUS inner 一、二、三 document-clause lists (the two
  agreements, p#135-137/p#141-143) that are ordinary paragraphs, NOT headings.

## What is NEXT

- Batch B22 = ch28. Kickoff is the paste-block at the top. Runs to completion (no gate); ends by
  pasting the B23 kickoff. B23 = ch29 (第十章 (上), a two-part chapter split ch29/ch30; confirm
  title_en in book.json). Working batch labels run ONE AHEAD of book.json's batches array from ch24
  on (ch24 = B18 … ch27 = B21, ch28 = B22).
- The frozen register reference is `reference/B01_frozen.md`. Narrative sits at 4.55-4.78 en/han;
  document-heavy chapters run higher (ch24 5.33, ch25 4.97, ch26 4.98, ch27 4.82) - alignment/register
  are the gates, not the raw ratio.
- Sub-heading pattern DIFFERS by chapter. Styles seen: Part One numbered 一/二/三; ch11/ch14/
  ...
  numbered-in-parens (一)/(二)…; ch27/ch28 use 一、二、三 enumerated headings (and ch28 also has INNER
  ...
  one ending in `」`), ch27 (two tail-glued, one ending in `」`). Grep each new chapter p-by-p, and
  DISTINGUISH enumerated LIST items / document clauses (per parity) from 一、 / (一) SECTION headings
  and from run-in labels.

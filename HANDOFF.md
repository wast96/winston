...
State: Batch B07 (ch32 through ch36: Chapters 32 to 36) is COMPLETE, checked, and
committed. The cumulative EPUB `out/The Whistling Wind.epub` has 37 of 60 chapters
translated (ch00 through ch36); the other 23 still link to their skeleton outlines
and the TOC stays fully navigable. Next up: Batch B08 = ch37 through ch41
(Chapters 37 to 41).

## Message to paste into the next chat

```
Whistling Wind B08 — Chapters 37 to 41 (ch37 through ch41).

Read CLAUDE.md in full (the working rules at the top are non-negotiable), then
HANDOFF.md, then book.json. Then do Batch B08 = units ch37 through ch41
(Chapters 37 to 41 of the novel; ~17,945 source chars) end to end, following the
...
- Read the batch's source text from data/src/ for each unit (ch37 =
  41_chapter39.txt, ch38 = 42_chapter40.txt, ch39 = 43_chapter41.txt, ch40 =
  44_chapter42.txt, ch41 = 45_chapter43.txt). Quote the source VERBATIM in the
...
- Author one aligned bilingual QC file out/<id>_bilingual.md per unit (a first
  ...
  (pass the source chapter heading as the third argument, e.g. "三十七"). WATCH the
  ...
  ALSO watch for source dittography (a clause printed twice, as in B07 ch35): quote
  it verbatim (both copies) in the QC file for parity, but render it ONCE in the
  reading text and list it in PROGRESS.
- Run the checks and record them in PROGRESS.md: check_numbers.py --noise
  ...
  B07 did in scratch/b07_check.json; scratch/ is gitignored, so regenerate it — and
...
- Number-check notes: the built-in NOISE strips clock times AND whole-hour "点钟"
  ...
  两点钟 / 两分钟 now strip whole (before the built-in 十分 "=very" can eat the "十分"
  out of "四十分"). The bare-一 measure patterns AND the two 一-idiom patterns all
  carry a negative lookbehind, so compound numbers like 十一个 / 十一点钟 / 十一日 /
  十一时 survive (B04). B06 added, at the TOP block, r"[一二三四五六七八九]十多"
  (mirroring the existing 四十几): the built-in r"[十几幾]多" was eating the "十多" out
  of 五十多 ("fifty-odd") and orphaning a 五 read as 5. WORD_NUM has gained the
  day-of-month / century ordinals "eleventh" (11), "seventeenth" (17), "twentieth"
  (20), "twenty-third" (23) for cases where \btwenty\b does not reach inside the
  spelled ordinal; add the next such ordinal the same way. Prefer noise/prose fixes
  over editing the script (a spelled "three hundred and forty" parses to 3/40/300, so
  write such figures as digits; a hyphen-joined "twenty-fourmo" parses only to 20, so
  write "twenty-four-mo"; write 十来个 as "ten or so", not "a dozen or so", so the
  source 10 survives). LIVE GOTCHAS: the bare-一 measure rule strips "一次" out of the
  ...
  check, so render them faithfully in the prose yourself; clock times, durations,
  positional years (一九四一年), 里 speeds, and bow angles likewise are not re-verified.
  ...
  ("all things"; 万≠10000), 两两三三, 畸零 ("odd/isolated"; 零≠0), 万国 (万国公墓;
  万≠10000); and the B07 additions 凋零 ("wither/fall"; 零≠0), 四顾 ("look all around";
  四≠4), 万岁 ("long live/banzai"; 万≠10000), 百般 ("in every way"; 百≠100).
- Footnotes into notes.json (about 3 per chapter-equivalent; anchors must be
  ...
  by the builder, so just append to each unit's list; B07 ended at note 60.
  Recurring subjects already have their note at first appearance in B01-B07 (Stephen,
  ...
  the Great World, and the B07 references Jiangwan / the Greater Shanghai civic centre,
  the Greater East Asia Co-Prosperity Sphere / "goddess of peace" propaganda, the
  heroine of Ghost Love, and the 小鹿乱撞 "little deer" idiom), so do NOT re-note them;
  footnote only genuinely new refs.
- Glossary discipline: glossary.json (105 rows) already fixes the whole cast and the
  Shanghai/Hangzhou geography, dance halls, restaurants, the cheongsam (旗袍), and the
  B07 additions (Miko 米可, Jiangwan 江湾, the Kaidi Restaurant 凯第饭店, the White Palace
  白宫舞厅, Greater East Asia 大东亚), on top of the whole B06 set (Nomura 野村, Rear
  Admiral Umetake 梅武, Colonel Arita 有田, Takeshima 武岛, Honsa Jiro 本佐次郎, Chen Ji
  陈寂, Dr. Gaolang 高朗, the Hailin Broadcasting Station 海邻广播电台, the Racecourse
  跑马厅, the International Cemetery 万国公墓, Jing'an Temple 静安寺, Malang Road 马浪路,
  Beiping 北平, the Great World 大世界). REUSE those exact renderings; add a new row for
  every new proper noun / place / org / term with status (attested / provisional /
  decided) and attestation, deciding the one rendering before you romanize it.
  Fact-check any historical/real-world reference against real scholarship (Wikipedia /
  Baidu Baike / academic), never LLM-generated sources (never Grok/Grokipedia), and say
...
- Commit on the one working branch claude/the-whistling-wind. Rewrite HANDOFF.md
  so its first section is the paste-ready kickoff for Batch B09 = ch42 through ch45.
...
- Deliverable filename: `out/The Whistling Wind.epub`. Working branch:
  ...
  the stray branch, per CLAUDE.md rule 2. (B01-B07 were each started on a stray
...
- Pipeline artifacts that persist across batches: glossary.json (105 rows),
  notes.json (60 notes so far), data/noise.txt (project number-check noise),
  ...
  survive; the top clock/duration classes include 两/兩 as of B07 so 两点四十分
  strips; WORD_NUM carries the day-of-month/century ordinals; see the B05 一次两次
  gotcha above), data/src/ (all source text, regenerated by the ingest; gitignored),
  data/zh/ (parity source, regenerated per unit by split_bilingual.py), scratch/
  (gitignored; put the check_structure config here, e.g. b07_check.json).

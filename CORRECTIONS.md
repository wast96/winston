...
### [GLOBAL] Footnote-density pass ordered (after the R1-R9 register revision)
Unit: book-wide
Where: the whole EPUB, once the register revision (R1-R9) is complete
...
Fix: run an additional final pass that GREATLY increases footnote density —
  ...
  is a reader model, not a quota, but be generous. Authored as the single F0
  footnote-density wave in REVISION_PLAN.md §12 (its own schedule + kickoff
  canon + gates). Sequencing: this pass runs AFTER R9; when R9 finishes the
  register revision, the R9 reply serves the F0 kickoff instead of a "book
  complete" notice. Carried forward in HANDOFF.md until the footnote pass is
  itself complete.
Status: PENDING — starts after R9 (one wave, F0).

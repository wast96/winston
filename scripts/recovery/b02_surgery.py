...
import sys
MISSING=[]   # anchors not found under the current OCR (skip-and-warn, not fatal)
...
def merge_after(P, suf):
    ...
    if i is None or i+1>=len(P):
        MISSING.append("merge_after:"+suf); return
    j=i+1
...
def split_at(P, sub, keep_with_first):
    ...
    if i is None or keep_with_first not in P[i]:
        MISSING.append("split_at:"+sub); return
...
i=find_end(P,"三等")
if i is not None: P[i]=P[i]+'客车。”'
else: MISSING.append("append:三等")
...
if i is not None:
    while "难以忘怀" not in P[i] and i+1 < len(P):
        P[i]=P[i]+P[i+1]; del P[i+1]
else:
    MISSING.append("joinquote:在上海期间")
...
save("ch03",P)
if MISSING:
    print("b02 surgery: %d anchor(s) not found under current OCR:" % len(MISSING))
